...
unsigned _comdat_install(unsigned EAX, SYMBOL_STRUCT **pECX)
{
	...
	--EAX;
	if (EAX >= LNAME_LARRAY._LARRAY_LIMIT)
	    _index_range();
	unsigned ESI2 = EAX;
	EAX >>= 12;
	ESI2 &= 4 * 1024 - 1;
	unsigned **EAX2 = (unsigned **)LNAME_LARRAY._LARRAY_BASES[EAX];
	unsigned *ESI = EAX2[ESI2];

	unsigned *EDI = &SYMBOL_HASH;
	EAX = COMDAT_FLAGS & CD_LOCAL;

	unsigned length = ESI[2];	// LENGTH
	unsigned EDX = ESI[1];		// HASH

	SYM_HASH_MOD = (COMDAT_FLAGS & CD_LOCAL) | ESI[0];	// MODULE # IF LOCAL
	EDI[1] = length;	// set SYMBOL_LENGTH
	EDI[0] = EDX;		// set SYMBOL_HASH

	memcpy(EDI + 2, ESI + 3, ((length >> 2) + 1) * 4);	// set SYMBOL_TEXT[]
	EAX = _far_install(EDX, pECX);
	SYM_HASH_MOD = 0;
	return EAX;
}
...
SYMBOL_STRUCT *_do_near_install(SYMBOL_STRUCT *EBX, SYMBOL_STRUCT **pECX)
{
	// DS:BX GETS POINTER
	SYMBOL_STRUCT *ESI = EBX;

	SYMBOL_STRUCT *EAX = (SYMBOL_STRUCT *)_tillmiddle_pool_get(SYMBOL_LENGTH + sizeof(SYMBOL_STRUCT) - 3);
	EBX = EAX;
	ESI->_S_NEXT_HASH_GINDEX = EAX;

	unsigned EDX = ((unsigned *)(SYMBOL_TEXT))[-1] >> 2;

	memset(EBX, 0, offsetof(SYMBOL_STRUCT, _S_NAME_TEXT));
	memcpy((char *)EBX + offsetof(SYMBOL_STRUCT, _S_NAME_TEXT), SYMBOL_TEXT, (EDX + 1) * 4);

	EBX->_S_REF_FLAGS = S_SPACES;
	*pECX = EBX;
	return EAX;
