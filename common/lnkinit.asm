...
XSEG		ENDS
		...
		EXTERNDEF	_NONRES_LENGTH_SEM:QWORD,GLOBAL_SEMAPHORE_LIST:QWORD,LOCKED_STUFF:LKD_ALLOC_STRUCT,MDB_GARRAY:QWORD
...
if	fg_cvpack
		...
		EXTERNDEF	OPENT:PROC,READT:PROC,STORE_FILNAM:PROC,INIT_ARRAYS:PROC,SET_TINY:PROC,SET_BINARY:PROC
		...
		EXTERNDEF	SIGNON:PROC,_get_filename:proc,SET_CASE_MODE:PROC,DEFINE_CLASS:PROC,DO_NEW_SECTION:PROC
		...
		EXTERNDEF	THREAD_FAIL_ERR:ABS,GLOBAL_SEMAPHORE_COUNT:ABS,CMDLINE_ERR:ABS,CREATESEM_ERR:ABS
...
if	fg_winpack
		...
		SETT	SUPPORT_@_INDIRECT

		CALL	SET_CASE_MODE			;SET DEFAULT CASE SIGNIFICANCE MODE
if	fgh_anythreads
		CALL	_init_semaphores
endif
		;
		;INITIALIZE STD ARRAYS
		;
;		CALL	INIT_STD_ARRAYS
...
if	fgh_win32dll
		...
		PUBLIC	CLOSE_SEMAPHORES

CLOSE_SEMAPHORES	PROC	NEAR
		;
		...
		MOV	ESI,GLOBAL_SEMAPHORE_COUNT
		MOV	EBX,OFF GLOBAL_SEMAPHORE_LIST
...
if	fg_symc
		...
		DB	0DH,0AH
		DB	'Copyright (C) Digital Mars 1989-2009'
